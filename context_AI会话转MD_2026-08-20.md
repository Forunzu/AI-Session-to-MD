...
> 最后更新：2026-09-04 | 当前阶段：v4.5 修掉用户实测到的「大备份点不动」竞态（点两次确认的第二下不再重跑干跑 + 待确认窗口 20s→60s + 签名防过期清单 + 异步期间禁按钮），exe 已按 v4.5 重打包并冒烟通过，四个探针全绿；代码 `f0d34ac` + context `092c8cd` 已推送，**Release v1.1.1 已发布（附件是修好的 exe，字节数与本地一致）**。剩下：真机续聊确认（需人在场）→ 换机搬运（CLI 数据 8.79 GB 走本工具备份）→ 新 exe 手感实测
...
## ✅ 当前进度
- 已完成：v1（解析/扫描/转换 + GUI + 单文件 exe）→ v2（指令目录 + MD 自动标题 + 单条导出）→ v3（双日期显示 + 手动排序 + 目录高亮 CSS 修复 + 开源发布）→ **v4（迁移功能：跨 CLI 续聊 + 换机备份还原，后端 3 个新模块 + 9 条新路由 + 迁移弹窗，已自测通过）** → **v4.1（真机实测反馈修复：续聊命令改发会话 id、备份目标选到盘根自动改用 `AI-CLI-Backup` 子目录、zip 落点与失败隔离、还原目录自动下钻）** → **v4.2（备份包结构与命名 + 还原认路：带日期的快照目录、zip 套同名顶层文件夹、`备份说明.txt`、`locate_backup` 覆盖四种选法、0 文件给原因并拒绝开跑、续聊命令去掉 `&&`）** → **v4.3（会话目录重做：Claude 侧去噪 + 重试折叠、目录列「我的指令 + 每轮 AI 首条回复」、🧑/🤖 分色、全部/只看我/只看 AI 三视图、scroll-spy 与 sticky 头修复）** → **v4.4（去噪补全覆盖两个 CLI 家族：噪音前缀 9 → 20 条（新增 11 条）、`parse_codex` 改「打标记不丢弃」兑现「不删只藏」、`scanner._peek()` 修列表脏标题；两个探针参数化成长期资产）** → **v4.5（用户实测反馈修复：「点两次确认」的第二下不再重跑干跑，第一下的结果缓存到按钮上；待确认窗口 20s → 60s；`bkSig`/`rsSig` 签名守住「确认的就是看到的那份」；`busy()` 在异步期间禁按钮防并发；预览按钮改走 `bkPreview`/`rsPreview`/`rwPreview`；新增第 4 个探针 `probe_vault_ui.js` 21 条断言）**。
- 进行中：换机收尾。v4.5 改的是 `web/app.js` + `web/index.html`（提交 `f0d34ac`，context 提交 `092c8cd`，两个都已推送、0 个未推送），已用 `probe_vault_ui.js` 21/21 全绿验证，并拿 `git show HEAD:web/app.js` 反向复现过修复前的死锁（干跑 2 次 / 起备份 0 次）；exe 已按 v4.5 重打包（`dist\会话转MD.exe` 21563915 字节），`probe_exe.py` 冒烟通过（6 条 HTTP 全 200、`/app.js` 4 个特征串都在、`/api/sessions` 125 个会话脏标题 0 个、45 个 CLI），四个探针全部通过。**Release v1.1.1 已发布**（用户拍板补发新 tag 而非原地换 v1.1.0 的附件），附件 `ChatToMD.exe` 字节数 21563915 与本地 exe 逐字节一致；v1.1.0 的附件（21563378 字节）仍是修复前那版，两版都是「20.6 MB」，**只有字节数分得开**。换机交接清单见下方 `💻 换机交接` 章节。
...
- **Release v1.1.0（2026-09-04 发布，非草稿、target main、tag 指向 `b01e37b`）**：`ChatToMD.exe`（**21563378 字节**），
  https://github.com/Forunzu/AI-Session-to-MD/releases/tag/v1.1.0 。发布说明里带了「已知情况」一节，
  写明真机续聊尚未实测、拿不准就先用交接包通道，以及备份前要关掉对应 CLI。
  **⚠ 这个附件是 v4.5 修复前的 exe**：文件数多到干跑超过 20 秒时，「确认备份」那一下会被吞，大备份点不动。已被 v1.1.1 取代。
- **Release v1.1.1（2026-09-04 09:21Z 发布，非草稿、非预发布、target main、tag 指向 `092c8cd`、GitHub 上标 Latest）**：
  `ChatToMD.exe`（**21563915 字节**，与本地 `dist\会话转MD.exe` 逐字节一致），
  https://github.com/Forunzu/AI-Session-to-MD/releases/tag/v1.1.1 。
  纯补丁版，功能与 v1.1.0 相同，只修「大备份 / 还原点不动」；说明里写了三连跳现象、四条修法，
  并把 v1.1.0 的「已知情况」两条（真机续聊未实测、备份前先关 CLI）原样带过来。
  **两版都显示「20.6 MB」，要分辨手上是哪一版只能看字节数。**
...
### v4.5 2026-09-04
**触发原因：** 用户按 v4.2 的流程去做换机备份，**大备份根本点不动**，拍了三张截图：先是按钮变成「确认备份 133331 个文件 / 8.9 GB（再点一次）」，接着**自己跳回了「开始备份」**（干跑区还停在「正在统计…」），最后又弹回干跑清单并 toast「已生成干跑清单，请核对后再点一次开始」。
场景痛点：这个 bug 只在「干跑比待确认窗口还慢」时出现，也就是**只在真正的大备份上出现**——而大备份正是这个功能存在的理由。小目录点两下就跑，怎么点都复现不了；用户第一次拿它办正事（8.9 GB / 13 万文件换机搬运）就卡死在最后一步，而且屏幕上三次状态互相打脸，看起来像界面乱跳而不是「你的操作被吞了」。
**根因：** `bkStart()` 在「确认」那一下**又跑了一遍干跑**：13 万文件的干跑要 40 秒以上（本机 `plan_backup` 实测 47.7 秒），而 `arm()` 的待确认窗口只有 20 秒。窗口先到期把待确认状态解除（截图二就是这一刻），干跑返回后 `arm()` 发现 `_armed` 已是 false，就把这一下当成「第一次点」重新进待确认（截图三）。**每点一次都重复这个循环，永远开不了跑。** 已用一次性脚本抠出修复前的 `bkStart` 复现：第二下 = 干跑 2 次 / 起备份 **0** 次。`rsStart()` 同一个写法，同一个病。
**变更内容：**
- 修改 `web/app.js`：`arm()` 拆出 `disarm()`；待确认窗口 20s → **60s**（`ARM_MS`）——几十行的干跑清单 20 秒读不完，这是它当初被设短的另一半代价。
- 修改 `web/app.js`：`bkStart()` / `rsStart()` 把**确认分支前置**——先消掉待确认状态，直接用上一次那份干跑结果开跑，**不再重算**。确认这一下从此不含任何网络等待，窗口只用来给人读清单。
- 新增 `busy(btn, fn, label)`：干跑期间禁用按钮并显示「统计中…（大目录要几十秒）」。不禁的话点第二下会再起一次干跑，把待确认状态和已渲染的清单一起冲掉（截图一里「确认态 + 正在统计…」并存就是这么来的）。
- 新增 `bkSig()` / `rsSig()`：待确认期间改了勾选 / 目标目录 / 冲突策略 / 路径映射，就作废旧清单并提示重新核对。「点两次」的意义是**你确认的就是你看到的那份**，签名是这句话唯一的保障。
- `bkPlan()` / `rsPlan()` 把结果缓存到按钮上（并在开跑前 `disarm`）：先「预览（干跑）」再「开始备份」不用再等一遍 40 秒。
- 预览类按钮统一改走 `bkPreview` / `rsPreview` / `rwPreview`（各包一层 `busy`），`web/index.html` 三处 onclick 跟着改。
- 新增**第四个探针** `probe_vault_ui.js`（node，无依赖）：把 app.js 里那几个函数原样抠出来套假 DOM 跑时序，把 ARM_MS 压到 300ms、干跑压到 900ms，21 项断言覆盖「干跑比窗口慢时第二下必须真开跑」「窗口到期后必须重新核对」「改了条件旧清单作废」「预览过就复用」「干跑失败不留半截状态」。
- `probe_exe.py` 的特征串检查升级成「一个 URL 可以要求一串都在」，`/app.js` 现在要求 `function disarm(` / `function bkSig(` / `function bkPreview(`，`/` 要求 `bkPreview(this)`——延续 v4.4 那条「验 exe 要挑只有修复生效才会变的东西」。
**当前状态：** 已定稿（四个探针全绿；exe 已重打包 20.6 MB，包内 6 个特征串全在，`/api/sessions` 脏标题仍 0）。
**未解决的分歧：** Release v1.1.0 的附件是修复前那版 exe，**下载到的人大备份点不动**，是否补发 v1.1.1 等用户拍板。
...
### A. 必须带走的（U 盘）
> **2026-09-04 已 `git push` + 发 Release v1.1.0 之后，这张表大幅缩短**：代码、context、exe 全在 GitHub 上，
> 新机直接 `git clone` / 下载 Release 即可，**项目本身不需要 U 盘**。只剩下 GitHub 上没有的东西要搬。

| 内容 | 位置 | 说明 |
|---|---|---|
| **各 CLI 的会话与配置** | `~\.claude`、`~\.codex`、其余 `~\.<cli>` | **唯一真正必须搬的东西**，GitHub 上一点都没有。**用本工具自己的「🔀 迁移 → 备份/还原」搬**（这功能就是为这件事做的），别手拷。实测干跑：45 个 CLI「整个根目录 + 智能排除」= **146699 文件 / 8.79 GB**（不排除是 10.34 GB）；只要对话历史的话「仅会话数据」范围下 claude + codex = **26176 文件 / 2.59 GB**。U 盘按 ≥16 GB 备。 |
| **四个自检探针** | `E:\AI工作交付物\AI会话转MD换机\` 下 `probe_outline.py` / `probe_roundtrip.py` / `probe_exe.py` / `probe_vault_ui.js` | 合计约 17 KB。**故意放在仓库外**（它们读真实会话内容），所以 GitHub 上没有。换机后先跑这四个，能一次验完解析去噪、目录复算、导出标题、双向往返、7 条路由、**打包后的 exe**（`probe_exe.py` 会自己起 exe、找端口、打 6 条 HTTP、跑完 `taskkill` 清进程），以及**备份/还原「点两次」的状态机**（`probe_vault_ui.js` 只要 node，不起服务、不碰文件，把 `web/app.js` 里的函数原样抠出来套假 DOM 跑时序，21 条断言）。 |
| （可有可无）`.claude\settings.local.json` | 仓库内，被 `.gitignore` 挡着 | 几 KB 的本地权限白名单，不带也行，新机用着自然会重生。 |

**不用带了**：`E:\AI工作交付物\AI会话转MD换机\*.bundle` 两份（`20260903` / `20260904`）——`git push` 成功后
GitHub 就是权威副本，bundle 的兜底作用已经用不上，留着只会过期误导；`E:\AI-CLI-Backup`（打于 09-03 15:15，
不含 09-04 一整天的会话，**对会话数据而言已经是旧快照**，别拿它当备份用）。
**不能带**：`dist\config.json`（存的是旧机绝对路径，见下）；`build\`、`会话转MD.spec`、`__pycache__\`（重打包会重生）。

### B. 新机开工顺序
0. 取项目：`git clone https://github.com/Forunzu/AI-Session-to-MD`（代码 + 本 context 文件都在里面）；
   只想用不想改的话，直接去 Release v1.1.0 下 `ChatToMD.exe` 双击即可。
...
6. 跑 `set CHATMD_REPO=<新仓库路径>` 后依次执行四个探针（`probe_outline.py` / `probe_roundtrip.py` / `probe_exe.py`，以及 `node probe_vault_ui.js`），确认新机上一切正常。前两个只读会话、写入全走临时沙箱；`probe_vault_ui.js` 不起服务也不碰文件，只要装了 node 就能跑；`probe_exe.py` 会起 exe 并在结束时 `taskkill /IM 会话转MD.exe`，所以跑之前先关掉手动开着的实例（它自己也会先断言「没有同名进程在跑」）。

### C. 提醒
- **凭证会跟着走**：默认包含（用户拍板），换机免重新登录。代价是 **U 盘丢了等于账号泄露**，别把这个 U 盘随手借人；
  不想带就在备份页逐行关掉。**旧机干跑实测，真正会被带走的凭证只有两个文件**：`codex\auth.json` 与
  `lobehub-market\credentials.json`——`~\.claude\.credentials.json` **在本机并不存在**（Claude Code 在 Windows 上
  把令牌放在别处），所以还原到新机后 Claude 侧仍然要重新登录一次，这是正常的，不是备份漏了。
- **备份别顺手勾 zip**：打包是先写目录再压缩，**两份同时躺在 U 盘上**，8.79 GB 会瞬间变成 ~13 GB 占用。
  直接备到 U 盘就别勾 zip；要 zip 就先备到本机硬盘、压完再把 zip 拷进 U 盘。
- **提交已经不是风险项了**（2026-09-04 已 `git push`，`git log --oneline origin/main..HEAD | wc -l` = 0）。
  之后若又在旧机改了东西，出发前再数一眼这条命令；只要它是 0，`.git` 和 bundle 都不用管。

### D. 换机后这些记录会立刻过期
以下都是**旧机实测值**，新机上要重新测，别当结论用：主机名 `PC-20250303XWLM`、HOME `C:\Users\Administrator`；探测到 45 个 AI CLI 目录；`.claude` 742 MB→399 MB、`.codex` 7.7 GB→3.1 GB；**45 个 CLI 干跑合计 146699 文件 / 8.79 GB（智能排除后，原始 10.34 GB），「仅会话数据」范围 claude+codex 26176 文件 / 2.59 GB**；体积前几名 Codex 3187.8 MB、WorkBuddy 1545.5 MB、CC Switch 1199.3 MB、CLI Manager 723.6 MB、ZCode 617.8 MB、Grok CLI 532.3 MB、Claude Code 423.2 MB；列表扫到 125 个会话；旧机 E 盘残留 `E:\AI-CLI-Backup`、`E:\claude`、`E:\AI-CLI-Backup_20260903-151509.zip`（**这三个在旧机上，换机前自行决定删不删，别指望它们出现在新机**）；待验的续聊会话 id `01a0661c-de9e-7006-b62c-fd43a71f85fe`（对应 cwd `E:\在办项目\脚本管理软件开发`，得先把 `.codex` 还原过去这条命令才有意义）。
...
- [v4.4/换机备份] `~\.claude\.credentials.json` **在本机根本不存在**，凭证干跑只报出 `codex\auth.json` 与 `lobehub-market\credentials.json` 两个。Claude Code 在 Windows 上不把令牌落成这个文件（放在系统凭据里）。→ 「带凭证就免重新登录」这句话对 Codex 成立、对 Claude 不成立；还原到新机后 Claude 侧仍要登一次，**这是正常现象而不是备份漏了**，写在换机交接 C 节里免得下次误判。规则里那条 `secrets: [".credentials.json"]` 保留不动（有就带、没有就跳过，`plan_backup` 已按 `os.path.exists` 过滤）。
- [v4.4/体积有两条口径] `cli_registry.measure()` 报 CLI Manager 735.9 MB，`vault.plan_backup()` 干跑报 723.6 MB，同一个目录差了 12 MB。measure 走的是目录递归累加，plan 走的是「真会被复制的那批文件」逐个 `stat`，排除规则的应用时机不同。→ **对外只报干跑值**（那才是真会占 U 盘的字节数），measure 的数字只用来在勾选列表里给个量级参考。

- [v4.5/待确认窗口和后端耗时抢跑] 「点两次才真跑」的第二下如果还要等一次后端计算，那这次计算的耗时就在跟自己的超时窗口赛跑：干跑 40 秒 > 窗口 20 秒，确认永远落不了地，**大备份彻底点不动**，而小目录怎么点都正常。→ 确认分支必须前置：先消状态、直接用上一次的结果，确认这一下不含任何网络等待。**凡是「二次确认」型交互，第二次点击必须是纯本地动作。**
- [v4.5/异步期间不禁按钮 = 状态互相打脸] 干跑在飞的时候按钮还能点，第二下又起一次干跑，把待确认状态和已渲染的清单一起冲掉——用户看到的是「确认态 + 正在统计…」并存、然后按钮自己跳回原文。三张截图里那种「界面乱跳」几乎都是这个成因。→ 慢操作一律 `busy()` 包一层：禁用 + 换文案 + `finally` 复原。
- [v4.5/两次确认之间条件可能已经变了] 待确认那 60 秒里用户完全可以改勾选、改目标目录、改冲突策略。拿旧清单开跑就等于「确认的不是看到的那份」，而这正是二次确认要防的事。→ 把输入算成签名存在按钮上，确认时比一次，不一致就作废并说清是什么变了。
- [v4.5/Git Bash 里 tasklist 的 `/FI` 会被当路径] `tasklist /FI "IMAGENAME eq 会话转MD.exe"` 在 Git Bash 里报 `无效参数/选项 - 'C:/Program Files/Git/FI'`——MSYS 把 `/FI` 当成了 Unix 路径去转换。我第一次查「exe 是不是在跑」时还带了 `2>/dev/null`，把这条报错吞了，于是把「命令失败」误读成「没有进程」，紧接着打包就 `WinError 5`。→ **查进程走 Python 的 `subprocess.run(['tasklist', '/FI', …])`**（不经 MSYS 路径转换，`probe_exe.py` 一直是这么写的才没踩到）；**别给状态查询加 `2>/dev/null`**，失败和空结果必须能分开。
- [v4.5/文件没被锁也可能 WinError 5] `PyInstaller` 报 `PermissionError: [WinError 5] dist\会话转MD.exe`，但同一时刻 `mv` 改名却成功——「能改名」只说明目录项没锁，不等于能删。真因还是那两个 exe 进程在跑（见上条）。→ 打包前用 Python 查一遍同名进程再 `taskkill`，别拿改名当「没被占用」的证据。
...
- [2026-09-04 收尾] Release v1.1.0 **不再压着等真机续聊**，改成「照发 + 在发布说明里写明未实测项」→ 用户当场拍板放开闸门（原约定是压到真机确认之后）。压着发的收益只是「不让人踩到没验过的功能」，但同样的目的用一节「已知情况」也能达到，而且更诚实：明写「真机 `--resume` 未实测、拿不准就先用交接包通道」，用户自己就能选风险。压着的代价反而是真实的——换机在即，压过这个窗口就没人发得出来了。
- [2026-09-04 收尾] `git push` 成功后，两份 `*.bundle` 主动判为「不用带、可删」→ bundle 的全部价值是「万一 `.git` 没拷全」的兜底，远端有了权威副本这层就是纯冗余，而它**每提交一次就落后一次**，留着的净效果是「以后有人拿旧快照当完整历史」。同理 `E:\AI-CLI-Backup`（09-03 15:15 那份）对会话数据已经是旧快照，写进 context 时直接标「别拿它当备份用」。
- [2026-09-04 收尾] 换机备份**建议不勾 zip**（如果直接备到 U 盘）→ 打包是「先写目录、再压缩到同级」，两份同时在盘上，8.79 GB 会顶到 ~13 GB 占用，U 盘写满的失败形态还发生在整件事的最后一步。要 zip 就先备到本机硬盘、压完再拷 zip 过去。这条不改代码，只写进交接章节。
- [2026-09-04 收尾] 给用户的备份体积一律用 **`plan_backup` 干跑值**，不用 `measure()` 的目录累加值 → 两者差着排除规则的应用时机（见踩坑同日条），只有干跑值等于「真会占 U 盘的字节」。顺带确立：凡是要用户据此做决定（买多大 U 盘、够不够放）的数字，必须来自和真实执行同一条代码路径。

- [2026-09-04 v4.5] 「点两次确认」的**第二下必须是纯本地动作**，一次后端调用都不许有 → 这是 v4.5 那个 bug 的根因抽象：第二下只要 await 后端，耗时就和待确认窗口赛跑，后端慢于窗口时确认必然被吞（本例 47.7s vs 20s，100% 触发）。改法是第一下把干跑结果缓存到按钮上（`_plan`），第二下只读缓存 + 起 job。否掉「把窗口再拉长」——那只是把 100% 必坏改成「文件更多时又坏」，赌的是用户的机器有多快。
- [2026-09-04 v4.5] 任何会 await 的按钮动作都要先禁按钮（`busy()` 统一包一层）→ 不禁的话第二下会并发起第二次干跑，两条时间线互相覆盖状态，正是截图里「按钮自己跳回原文，几十秒后清单又冒出来」的成因。顺带解决「按钮上该显示什么」：`busy` 记原文 → 显示「统计中…」→ `finally` 还原，`arm()` 就不会把「统计中…」误当原文。
- [2026-09-04 v4.5] 待确认期间用**签名**（`bkSig` / `rsSig`：勾选 + 目标目录 +（还原侧）冲突策略 + 映射）守住「你确认的就是你看到的那份」→ 窗口拉到 60 秒后，用户完全来得及改完勾选再点确认，此时旧清单已经不描述将要发生的事。签名对不上就作废并提示重新核对，代价是多点一次，收益是不会拿一份过期清单往真实 HOME 里写。
- [2026-09-04 v4.5] 前端状态机用**抠函数 + 假 DOM + 压缩时序**的探针测，不靠起服务点按钮 → 这个 bug 的触发条件是「后端比超时慢」，真机上要凑 13 万文件才稳定复现，点一轮 40 秒以上。`probe_vault_ui.js` 把 `ARM_MS` 压到 300ms、干跑桩固定 900ms，同一个竞态毫秒级重现，21 条断言全绿；再拿 `git show HEAD:web/app.js` 跑一遍得「干跑 2 次 / 起备份 0 次」，**先证明 bug 真实存在，再证明修复有效**。
- [2026-09-04 v4.5] 打包产物的验证一律**看输出、不看源码**：web 资源用 HTTP 取回比对 ASCII 特征串（`function bkSig(`、`function bkPreview(`、`bkPreview(this)`），编进 PYZ 的 `parser`/`scanner` 只能用输出数字反证（脏标题 0 条）→ exe 里翻不到源码，「我改了仓库」不等于「用户拿到的那个 exe 改了」。特征串挑纯 ASCII 是为了绕开响应编码差异造成的假失败。
- [2026-09-04 v4.5] 修复后**补发 Release v1.1.1（新 tag + 新附件），不原地换 v1.1.0 的附件**（用户拍板）→ 同一个版本号对应两份不同的 exe 之后，谁也没法判断某人手上那份修没修，Issue 里一句「我用的 v1.1.0」就再不含信息量。代价只是多一个 tag。本例尤其明显：两份 exe 都显示「20.6 MB」，**只有字节数（21563378 vs 21563915）分得开**，版本号是唯一还能指望的标识。顺带确立发布后的核对动作：比对远端附件字节数与本地 exe 是否逐字节一致，只看「上传成功」不算验过。

---

## 🔜 下一步（优先级排序）
> 换机在即（2026-09-04），顺序按此重排过：**先把不可再生的东西弄安全，再做需要人在场的验证。**
> **已划掉三条**：`git push`（v4.4 及以前 14 个提交 `d797007..b01e37b`，v4.5 再加 `f0d34ac` + `092c8cd`，现在 0 个未推送）、
> **Release v1.1.0**（已发布），以及 **Release v1.1.1**（已发布，附件是修好的 exe，字节数与本地一致，GitHub 标 Latest）。
> 原来「Release 压在真机确认之后」的闸门由用户当场拍板放开，发布说明里用「已知情况」一节把
> 「真机续聊未实测、拿不准先用交接包通道」写在明面上，替代了那道闸门；v1.1.1 也照样带了这一节。

1. **真机续聊确认**（需用户在场，这是唯一还没在真 CLI 里验过的一环；**换机前做完最省事**，
...
2. **换机搬运**（GitHub 已经是权威副本，所以 U 盘只剩两样东西）：
   ① **各 CLI 数据**——本工具 🔀 迁移 → 备份，45 个 CLI「整个根目录 + 智能排除」干跑 146699 文件 / 8.79 GB，
   U 盘按 ≥16 GB 备；**直接备到 U 盘就别勾 zip**（目录 + zip 两份同时占盘会到 ~13 GB）；**开跑前先关掉各 CLI**。
   ② **四个探针**（`E:\AI工作交付物\AI会话转MD换机\`，约 17 KB，故意在仓库外）。
   新机第一件事：`git clone https://github.com/Forunzu/AI-Session-to-MD` → `pip install -r requirements.txt`
   → **删掉或改掉 `dist\config.json`**（里面是旧机绝对路径）。
3. **新 exe 手感实测**（`dist\会话转MD.exe` 已按 v4.5 重打包：21563915 字节，`probe_exe.py` 冒烟通过——
   6 条 HTTP 全 200、包内 web 资源是新版（`function bkSig(` / `function bkPreview(` / `function disarm(` 都在）、
   `/api/sessions` 125 个会话**脏标题 0 个**，说明修复确实在用户会拿到的那个 exe 里；Release v1.1.1 的附件与它逐字节一致。
   剩下的是只能靠手点的部分）。双击后一次把 v4.2 + v4.3 + v4.4 + v4.5 都验掉：
   ...
   ⑥ **v4.5 专项**（就是这次报的 bug，全选 45 个 CLI 才压得出原来的时序）：点「开始备份」→ 统计期间按钮禁用显示
      「统计中…（大目录要几十秒）」→ 出清单后按钮变「确认备份 N 个文件 / X GB（再点一次）」→ **慢慢核对清单**
      （现在有 60 秒）→ 再点一次**必须立刻开跑**，不能又变回「开始备份」；另外试一次「先点预览（干跑）、再点开始备份」，
      确认不会重算第二遍；再试一次「进了待确认之后改掉勾选或目标目录」，应提示「已作废刚才那份清单」而不是照旧开跑。
4. **清理 E 盘残留**（用户的数据，等用户拍板；**这些东西在旧机上**，换机后这条自动作废）：
   ...
   另外 `E:\AI工作交付物\AI会话转MD换机\*.bundle` 两份在 push 成功后已失去兜底意义，可一并删。
5. 收集使用/社区反馈（GitHub Issues + 本机试用），继续调界面 / 导出格式 / 分组排序。
6. （可选）若需彻底零 WebView2 依赖，再打包固定版运行时。
7. （挂着）那 1 条脏标题（`# 全局工作规范 …`）等有可靠判据再修，理由见决策日志 2026-09-04 条。
8. （待用户拍板，可选）要不要在 **v1.1.0 的发布说明**顶上补一行「这一版有此 bug，请下 v1.1.1」——
   附件按用户决定保留不动，只加一行指路。不加也行（v1.1.1 已经是 GitHub 标的 Latest，仓库首页的下载链导向它）。
...
### 2026-09-04 推送 + 发 Release + 算清换机要备份什么
**本次做了什么：** 用户一句话把两件压着的事一起放开：「推提交同时发 RELEASE 吧」，并问「我现在要备份的话需要做哪些事，还是什么都不用、后续直接从 GitHub 调取？」
① **`git push origin main`**：14 个提交 `d797007..b01e37b` 全部推上，`git log --oneline origin/main..HEAD` 现在是 0 条。
② **发布 Release v1.1.0**：`gh release create v1.1.0 "dist/ChatToMD.exe" --target main`，非草稿、非预发布、tag 指向 `b01e37b`、
   附件 `ChatToMD.exe` 20.5 MB（中文文件名会被 GitHub 过滤，所以另存了 ASCII 名，上传完就删掉，避免 `dist\` 里留两个同源 exe 让人分不清）。
   发布说明里留了一节「已知情况」：真机 `--resume` 未实测、拿不准先用交接包通道、备份前先关掉对应 CLI。原来「Release 压在真机确认之后」
   那道闸门由用户当场放开，用这节说明替代。
③ **把「要备份什么」算成了确定数字**，不再是「大概几个 G」：`vault.plan_backup()` 干跑两种范围——
   45 个 CLI「整个根目录 + 智能排除」= **146699 文件 / 8.79 GB**（不排除 10.34 GB）；「仅会话数据」范围 claude + codex = **26176 文件 / 2.59 GB**。
   凭证干跑只报出两个文件（`codex\auth.json`、`lobehub-market\credentials.json`），`~\.claude\.credentials.json` 在本机不存在。
④ 按这些数字重写了 `💻 换机交接` A 节：push + Release 之后**项目本身不用进 U 盘**（`git clone` 或下 Release 即可），
   U 盘只剩「45 个 CLI 的数据」和「三个探针（约 13 KB，故意在仓库外）」两样；两份 `*.bundle` 与 09-03 那份 `E:\AI-CLI-Backup` 都标成「不用带」。
**关键结论 / 产出：** 远端成了权威副本，换机的不可再生项从「未推送提交 + CLI 数据」缩到只剩 **CLI 数据**一项。
方法上确认一条：**给用户拿去做决定的体积数字，必须来自和真实执行同一条代码路径**——`measure()` 的目录累加值和 `plan_backup()` 的
干跑值在 CLI Manager 上差了 12 MB，只有后者等于真会占 U 盘的字节。另外确认「带凭证就免重登」这句话只对 Codex 成立，
Claude 在 Windows 上不落 `.credentials.json`，还原后仍要登一次——这不是备份漏了。
**遗留问题：** ① 备份还没真跑（等用户插 U 盘，**开跑前先关掉各 CLI**，直接备到 U 盘**别勾 zip**，否则峰值 ~13 GB）；
② 真机续聊确认仍是唯一没在真 CLI 里验过的一环，**建议换机前做完**；
③ 新 exe 的手点实测（备份/解压/三种还原选法、会话目录三视图与滚动高亮）仍挂着；
④ E 盘残留 + 两份 bundle 是否删，是用户的数据、等用户拍板，且都在**旧机**上。

### 2026-09-04 v4.5（用户实测截图报 bug：备份点不动）
**本次做了什么：** 用户连发三张截图——按钮先变成「确认备份 133331 个文件 / 8.9 GB（再点一次）」，接着**自己跳回**「开始备份」，
最后干跑清单又冒了出来。按状态机把三张图对回一个竞态并修掉：`bkStart()` / `rsStart()` 的**第二下（确认）**原来会
再 await 一遍干跑，13 万文件要 47.7 秒，而 `arm()` 的待确认窗口只有 20 秒——窗口必然先到期，`arm()` 把这一下当成
「第一次点」重新进待确认，确认永远落不了地。改法四条：确认分支放到函数最前面且只读缓存不再调后端；第一下把干跑结果
连同 `bkSig()` / `rsSig()` 签名缓存到按钮上；窗口 20s → 60s；新增 `busy()` 在异步期间禁按钮 + 换文案 + `finally` 还原。
预览按钮相应改走 `bkPreview(this)` / `rsPreview(this)` / `rwPreview(this)`。
验证：新写第 4 个探针 `probe_vault_ui.js`（node，把 `web/app.js` 的函数原样抠出来套假 DOM，`ARM_MS` 压到 300ms、
干跑桩 900ms）21 条断言全绿；再拿 `git show HEAD:web/app.js` 跑同一套，得「干跑 2 次 / 起备份 0 次」——**修复前确实死锁**。
exe 重打包后 `probe_exe.py` 冒烟通过（6 条 HTTP 全 200、`/app.js` 4 个特征串都在、125 个会话脏标题 0 个、45 个 CLI）。
**关键结论 / 产出：** 抽出一条通用规则——**「点两次确认」的第二下必须是纯本地动作，一次后端调用都不许有**，否则它就在和
自己的超时赛跑；窗口拉长只是把「必坏」改成「文件更多时又坏」。配套两条：任何 await 的按钮动作都要先禁按钮（不禁就会并发
起第二次，两条时间线互相覆盖状态，正是截图 2、3 的成因）；窗口拉长之后必须用签名守住「你确认的就是你看到的那份」。
方法上确认：**前端状态机可以不起服务就测**——抠函数 + 假 DOM + 把真实时序压到毫秒，这类「后端比超时慢」的 bug 才有稳定复现。
**遗留问题：** ① **Release v1.1.0 的附件是修复前那版 exe，下载到的人大备份点不动**，是否补发 v1.1.1 等用户拍板
（建议补发新 tag，不建议原地换附件——同号两份 exe 以后没法判断谁手上那份修没修）；
② 备份还没真跑（等用户插 U 盘，开跑前先关掉各 CLI，直接备到 U 盘别勾 zip）；③ 真机续聊确认仍是唯一没在真 CLI 里验过的一环；
④ v4.5 的手点实测（全选 45 个 CLI 压出原来的时序，走完「统计中→确认→立刻开跑」）挂着；
⑤ E 盘残留 + 两份 bundle 是否删，等用户拍板，都在旧机上。

### 2026-09-04 补发 Release v1.1.1
**本次做了什么：** 把 v4.5 的修复推上远端（`f0d34ac` 代码 + `092c8cd` context，`git log origin/main..HEAD` 数出来 0 条），
并按用户拍板**补发 Release v1.1.1**（新 tag，不原地换 v1.1.0 的附件）：把 `dist\会话转MD.exe` 拷成 `ChatToMD.exe`
上传，tag 指向 `092c8cd`、target main、非草稿非预发布、GitHub 标 Latest；发布说明写了三连跳现象（确认态 → 自己跳回
「开始备份」→ 清单又冒出来）、四条修法，并把 v1.1.0 的「已知情况」两条（真机续聊未实测、备份前先关 CLI）原样带过来。
发完核对：远端附件 **21563915 字节 == 本地 exe 21563915 字节**。
**关键结论 / 产出：** 补发新 tag 而不是换附件的价值在这次特别具体——两份 exe 都显示「20.6 MB」，
**只有字节数（21563378 vs 21563915）分得开**，版本号是唯一还能指望的标识；同号两份 exe 之后，Issue 里
一句「我用的 v1.1.0」就再不含信息量。顺带确立发布后的核对动作：**比对远端附件字节数与本地 exe，只看「上传成功」不算验过。**
**遗留问题：** ① 真机续聊确认仍是唯一没在真 CLI 里验过的一环（需人在场，**换机前做完最省事**）；
② U 盘备份还没真跑（开跑前先关掉各 CLI，直接备到 U 盘别勾 zip）；③ v4.5 的手点实测（全选 45 个 CLI 压出原来的时序）挂着；
④ 要不要在 v1.1.0 的发布说明顶上补一行「请下 v1.1.1」等用户拍板（附件保留不动，只加指路）；
⑤ E 盘残留 + 两份 bundle 是否删，等用户拍板，都在旧机上。
