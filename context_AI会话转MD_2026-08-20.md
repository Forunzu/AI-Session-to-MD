...
> 最后更新：2026-09-04 | 当前阶段：v4.5 修掉用户实测到的「大备份点不动」竞态（点两次确认的第二下不再重跑干跑 + 待确认窗口 20s→60s + 签名防过期清单 + 异步期间禁按钮），exe 已按 v4.5 重打包并冒烟通过，四个探针全绿；代码修复 `f0d34ac` 与本 context 已推上 GitHub。剩下：决定是否补发 Release v1.1.1（**v1.1.0 的附件是修复前那版 exe**）→ 真机续聊确认（需人在场）→ 换机搬运（CLI 数据 8.79 GB 走本工具备份）
...
- 进行中：换机收尾。v4.5 改的是 `web/app.js` + `web/index.html`（提交 `f0d34ac`，已连同本 context 推上 GitHub），已用 `probe_vault_ui.js` 21/21 全绿验证，并拿 `git show HEAD:web/app.js` 反向复现过修复前的死锁（干跑 2 次 / 起备份 0 次）；exe 已按 v4.5 重打包（`dist\会话转MD.exe` 20.6 MB），`probe_exe.py` 冒烟通过（6 条 HTTP 全 200、`/app.js` 4 个特征串都在、`/api/sessions` 125 个会话脏标题 0 个、45 个 CLI），四个探针全部通过。**Release v1.1.0 已发布，但附件 `ChatToMD.exe`（20.5 MB）是 v4.5 修复前那版——下载到的人大备份点不动，是否补发 v1.1.1 等用户拍板。** 换机交接清单见下方 `💻 换机交接` 章节。
...
- **Release v1.1.0（2026-09-04 发布，非草稿、target main、tag 指向 `b01e37b`）**：`ChatToMD.exe`（20.5 MB），
  https://github.com/Forunzu/AI-Session-to-MD/releases/tag/v1.1.0 。发布说明里带了「已知情况」一节，
  写明真机续聊尚未实测、拿不准就先用交接包通道，以及备份前要关掉对应 CLI。
  **⚠ 这个附件是 v4.5 修复前的 exe**：文件数多到干跑超过 20 秒时，「确认备份」那一下会被吞，大备份点不动。
  修好的 exe 在本机 `dist\会话转MD.exe`（20.6 MB）；是否补发 v1.1.1 见下一步第 1 条。
...
> **已划掉两条**：`git push`（v4.4 及以前 14 个提交 `d797007..b01e37b` 全部推上）与 **Release v1.1.0**（已发布，
> 非草稿、附件 `ChatToMD.exe` 20.5 MB）。原来「Release 压在真机确认之后」的闸门由用户当场拍板放开，
> 发布说明里用「已知情况」一节把「真机续聊未实测、拿不准先用交接包通道」写在明面上，替代了那道闸门。
> **v4.5 新加了第 1 条**：v1.1.0 的附件是修复前那版 exe，下载到的人**大备份点不动**。

1. **决定是否补发 Release v1.1.1**（等用户拍板，发布是对外动作）：v1.1.0 的 `ChatToMD.exe` 是 v4.5 修复前那版，
   拿到它的人只要备份的文件数够多（干跑超过 20 秒），确认那一下就会被吞、**大备份永远点不动**——正是用户这次截图里的现象。
   建议**补发 v1.1.1**（新 tag + 新附件），不建议原地换 v1.1.0 的附件：同一个版本号对应两份不同的 exe，
   以后没法判断某人手上那份修没修。本地 `dist\会话转MD.exe` 已是修好的版本（20.6 MB，`probe_exe.py` 冒烟通过）。
...
3. **换机搬运**（GitHub 已经是权威副本，所以 U 盘只剩两样东西）：
...
4. **新 exe 手感实测**（`dist\会话转MD.exe` 已按 v4.5 重打包：20.6 MB，`probe_exe.py` 冒烟通过——
   6 条 HTTP 全 200、包内 web 资源是新版（`function bkSig(` / `function bkPreview(` / `function disarm(` 都在）、
   `/api/sessions` 125 个会话**脏标题 0 个**，说明修复确实在用户会拿到的那个 exe 里。剩下的是只能靠手点的部分）。
   双击后一次把 v4.2 + v4.3 + v4.4 + v4.5 都验掉：
...
5. **清理 E 盘残留**（用户的数据，等用户拍板；**这些东西在旧机上**，换机后这条自动作废）：
...
8. （挂着）那 1 条脏标题（`# 全局工作规范 …`）等有可靠判据再修，理由见决策日志 2026-09-04 条。
