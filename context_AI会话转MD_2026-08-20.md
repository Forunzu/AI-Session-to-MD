...
> 最后更新：2026-09-04 | 当前阶段：v4.4 已把去噪补成两个 CLI 家族全覆盖（噪音前缀 9 → 20 条 + Codex 改「打标记不丢弃」+ 列表脏标题修完），exe 已按 v4.4 重打包（15:06）并冒烟通过，换机交接与三个可复用探针已就位；待 `git push` 12 个提交 → 真机续聊确认 → 换机 → 发 Release
...
| 内容 | 位置 | 说明 |
|---|---|---|
| **整个项目文件夹** | `E:\在办项目\AI会话转MD\` | **一定要连隐藏的 `.git` 一起拷**，12 个未推送提交都在里面。整夹约 53 MB（含 `dist\`、`build\`）。有些拷贝工具默认跳过隐藏目录，拷完到新机先 `git log --oneline -3` 核一眼。 |
| **仓库 bundle（双保险）** | `E:\AI工作交付物\AI会话转MD换机\AI会话转MD_全量仓库_20260903.bundle` | 117 KB，`git bundle --all` 打的完整历史（`main` = `c86b41e`，含 `origin/main` 指针），已 `git bundle verify` 通过。万一 `.git` 没拷全，新机 `git clone <bundle> AI会话转MD` 就能完整复原。**注意它只到 `c86b41e`，不含 v4.4 的两个提交**——要么换机前 `git push`，要么重打一份 bundle。 |
...
- **12 个未推送提交是唯一不可再生的东西**。最稳的做法是换机前先 `git push`（推提交 ≠ 发 Release，Release 可以继续压着等真机续聊验完）。不推也行，但 `.git` 和 bundle 至少要有一份到位，且 bundle 还得重打（现有那份只到 `c86b41e`）。
...
1. **换机前先 `git push`**（原来压在真机确认之后，现在提到第一位）：本地已积 **12 个未推送提交**
   （`59c31c0` v4 + `46abec7` context + `20c15d3` v4.1 + `10913a5` context + `fd38b17` context + `5430c3f` v4.2
   + `986e5f2` context + `020c359` context + `10cafb7` v4.3 + `c86b41e` context + v4.4 代码 + 本条 context）。
   这是整个项目里**唯一不可再生**的东西——exe 能重打、探针能重写、备份能重做，提交历史丢了就没了。
   **`git push` ≠ 发 Release**，推上去不会破坏「Release 压在真机确认之后」这个约定。
   兜底已就位：`E:\AI工作交付物\AI会话转MD换机\AI会话转MD_全量仓库_20260903.bundle`（117 KB，`git bundle verify` 通过），
   但它只到 `c86b41e`，v4.4 之后要么 push、要么重做一份 bundle。
...
③ 换机搬运与 `git push` 是这次之后最要紧的两件事，前者按 `💻 换机交接` 走，后者的理由是「12 个未推送提交是唯一不可再生的东西」；
